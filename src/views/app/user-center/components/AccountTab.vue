      ...
      <div class="flex justify-between items-start">
        ...
        <Dialog v-model:open="isEditingUsername">
          <DialogTrigger as-child>
            <Button variant="outline" size="sm" @click="editUsername">修改</Button>
          </DialogTrigger>
          <DialogContent class="sm:max-w-md">
            ...
            <div class="space-y-4 py-4">
              <div class="space-y-2">
                ...
                <Input id="username" v-model="newUsername" placeholder="请输入用户名" />
              </div>
            </div>
            <DialogFooter>
              <Button variant="outline" @click="isEditingUsername = false">取消</Button>
          ...
          <DialogContent class="sm:max-w-md">
            ...
            <div class="space-y-4 py-4">
              <div v-if="phoneStep === 'input'" class="space-y-2">
                ...
                <Input id="phone" v-model="newPhone" placeholder="请输入手机号" type="tel" />
              </div>
              <div v-else class="space-y-4">
                <div class="space-y-2">
                  ...
                  <p class="text-xs text-gray-500">验证码已发送至 {{ newPhone }}</p>
                </div>
                <Button
                  variant="outline"
                  size="sm"
            ...
            <DialogFooter>
              <Button variant="outline" @click="isEditingPhone = false">取消</Button>
              <Button v-if="phoneStep === 'input'" @click="sendPhoneOtp">发送验证码</Button>
              <Button v-else @click="savePhone">确认</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
    ...
    <div class="bg-white rounded-lg border p-6">
      <div class="flex justify-between items-start">
        ...
        <Dialog v-model:open="isEditingPassword">
          <DialogTrigger as-child>
            <Button variant="outline" size="sm" @click="editPassword">修改</Button>
          </DialogTrigger>
          <DialogContent class="sm:max-w-md">
            <DialogHeader>
            ...
            <div class="space-y-4 py-4">
              <div v-if="passwordStep === 'verify'" class="space-y-4">
                <div class="space-y-2">
                  ...
                  <p class="text-xs text-gray-500">验证码已发送至 {{ currentUser?.email }}</p>
                </div>
                <Button
                  variant="outline"
                  size="sm"
            ...
            <DialogFooter>
              <Button variant="outline" @click="isEditingPassword = false">取消</Button>
              <Button v-if="passwordStep === 'verify'" @click="verifyEmailOtp">验证</Button>
              <Button v-else @click="savePassword">保存</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
      ...
      <div class="space-y-4">
        ...
        <AlertDialog v-model:open="isDeleteDialogOpen">
          <AlertDialogTrigger as-child>
            <Button variant="destructive" size="sm" @click="startDeleteAccount">注销账户</Button>
          </AlertDialogTrigger>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction v-else class="bg-red-600 hover:bg-red-700" @click="deleteAccount">
                确认注销
              </AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
